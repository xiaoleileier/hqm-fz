...
// 过滤工单消息中的用户信息，只显示用户输入的内容
export function filterUserInfoFromMessage(message) {
  if (!message) return message;
  
  // 查找用户信息分隔符的位置
  const userInfoStart = message.indexOf('-------------');
  if (userInfoStart === -1) {
    return message; // 没有用户信息，直接返回
  }
  
  // 返回用户信息之前的内容
  return message.substring(0, userInfoStart).trim();
}
